        ...
        init: function(options) {
            ...
            return this.each(function() {
                ...
                if(settings.useFilters) {
                    elem_html += '<div id="' + filter_container_id + '" class="' + settings.filterContainerClass + '" role="dialog" aria-hidden="true">';

                    elem_html += '<div class="modal-dialog" role="document"><div class="modal-content">'
                    elem_html += '<div class="modal-header">'
                    elem_html += '<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>'
                    elem_html += '<h4 class="modal-title" id="exampleModalLabel">查询条件</h4>'
                    elem_html += '</div>'
                    elem_html += '<div class="modal-body">'
                ...
                if(settings.row_primary_key &&
                    ...
                    elem_table.off("click", "tbody tr").on("click", "tbody tr", function() {
                        ...
                        var row_id = $(this).attr("id").replace("bi_list_tr_", ""),
                            row_status,
                            ...
                            switch(sel_index) {
                                case 0:
                                    $(selector_table_tr).each(function() {
                                        row_id = $(this).attr("id").replace("bi_list_tr_", "");
                                        idx = methods.selectedRows.call(elem, "selected_index", row_id);
                                        if(idx == -1) {
                                            methods.selectedRows.call(elem, "add_id", row_id);
                                        }
                ...
                if(settings.useFilters) {
                    ...
                    elem_tools.off("click", "#" + filter_toggle_id).on("click", "#" + filter_toggle_id, function() {
                        ...
                        $(elem_filter_container).modal('toggle');

                        $(elem_filter_container).draggable({
					        cursor: "move",
					        handle: '.modal-header'
				        });
                    });
                    ...
                    if(s.row_primary_key && s.selected_ids.length > 0) {
                        ...
                        if(s.rowSelectionMode == "single" || s.rowSelectionMode == "multiple") {
                            ...
                            $("#" + table_id + " tbody tr").each(function() {
                                row_id = $(this).attr("id").replace("bi_list_tr_", "").replace("bi_list_tr_", "");
                                idx = methods.selectedRows.call(elem, "selected_index", row_id);
                                if(idx > -1) {
                                    methods.selectedRows.call(elem, "mark_selected", row_id);
                                }
